...
public class TweetService {
	...
	public List<TweetDisplay> findAll() {
		List<TweetDisplay> tweets = (List<TweetDisplay>) tweetRepository.findAllByOrderByCreatedAtDesc();
		return formatTweets(tweets);
	}

	public List<TweetDisplay> findAllByUser(User user) {
		List<TweetDisplay> tweets = tweetRepository.findAllByUserOrderByCreatedAtDesc(user);
		return formatTweets(tweets);
	}
	...
	private List<TweetDisplay> formatTweets(List<TweetDisplay> tweets) {
		addTagLinks(tweets);
		shortenLinks(tweets);
	...
	private void shortenLinks(List<TweetDisplay> tweets) {
		Pattern pattern = Pattern.compile("https?[^ ]+");
		for(TweetDisplay tweet: tweets) {
	...
	private void addTagLinks(List<TweetDisplay> tweets) {
		Pattern pattern = Pattern.compile("#\\w+");
		for(TweetDisplay tweet: tweets) {
			String message = tweet.getMessage();
			Matcher matcher = pattern.matcher(message);
	...
	public List<TweetDisplay> findAllWithTag(String tag){
		List<TweetDisplay> tweets = tweetRepository.findByTags_PhraseOrderByCreatedAtDesc(tag);
		return formatTweets(tweets);
	}
}
